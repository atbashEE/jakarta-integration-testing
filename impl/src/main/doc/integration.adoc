...
The JDBC Driver that will be installed together with your application on the runtime, s derived from the selected container.  You can override this through a configuration as described below.
...
- _jdbcDriverArtifact_ an annotation that allows to define which JDBC driver must be taken instead of the default one.  The `driverJarFile` expects an absolute path to the JAR file containing the driver. If specified, it overrules the `mavenArtifact` on that annotation which specifies the driver as maven artifact that must be present in the local repo. The format is `<groupId>:<artifactId>:<version>`. If neither are defined, the default configured within the Atbash code is taken.
